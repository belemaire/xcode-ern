 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addHeaderFile = function(p, opt, group) {
  var file
  if (group) {
    file = this.addFile(p, group, opt)
  } else {
    file = this.addPluginFile(p, opt)
  }
  ...
  if (path.extname(p) === '.swift') {
    if (opt && opt.public === true) {
      file.settings = { ATTRIBUTES: ['Public'] }
    }
    // Special handling for .swift files added as headers
    // to set proper comment ('in Headers' instead of 'in Sources',
    // being the default group for .swift files)
    var commentKey = f('%s_comment', file.uuid)
    this.pbxBuildFileSection()[file.uuid] = pbxBuildFileObj(file)
    this.pbxBuildFileSection()[commentKey] = f(
      '%s in %s',
      file.basename,
      'Headers'
    )
    ...
    headers.files.push({
      value: file.uuid,
      comment: f('%s in %s', file.basename, 'Headers'),
    })
  } else {
    this.addToPbxBuildFileSection(file) // PBXBuildFile
...
/**
 * Add a PBXGroup entry with its children being reference proxy uuids
 */
pbxProject.prototype.addPbxGroupWithReferenceProxyUuids = function({
  /**
   * An array of reference proxy UUIDs to be added as children of the group
   * REQUIRED
   */
  referenceProxyUuids,
  /**
  ...
  groupName,
  /**
   * The name of the source tree to use for the group
   * OPTIONAL [Default : "<group>"]
   */
  groupSourceTree,
  /**
   * The name of the path to use for the group
   * OPTIONAL [Default : No path]
   */
  groupPath,
}) {
  if (!referenceProxyUuids) {
    throw new Error(
      '[addPbxGroupWithReferenceProxyUuids] Missing required referenceProxyUuids'
    )
  }
  if (referenceProxyUuids.length === 0) {
    throw new Error(
      '[addPbxGroupWithReferenceProxyUuids] referenceProxyUuids cannot be an empty array'
    )
  }
  if (!groupName) {
    throw new Error(
      '[addPbxGroupWithReferenceProxyUuids] Missing required groupName'
    )
  }

  const referenceProxySection = this.pbxReferenceProxySection()
  const referenceProxyUuidsAndPaths = Object.keys(referenceProxySection)
    .filter(k => COMMENT_KEY.test(k))
    .filter(k => referenceProxyUuids.includes(k.split(COMMENT_KEY)[0]))
    .map(k => ({
      uuid: k.split(COMMENT_KEY)[0],
      path: referenceProxySection[k],
    }))
  ...
  const pbxGroup = {
    ...
    name: groupName,
    path: groupPath,
    sourceTree: groupSourceTree ? groupSourceTree : '"<group>"',
  }

  for (const referenceProxyUuidAndPath of referenceProxyUuidsAndPaths) {
    pbxGroup.children.push({
      value: referenceProxyUuidAndPath.uuid,
      comment: referenceProxyUuidAndPath.path,
    })
  }
...
pbxProject.prototype.addPbxGroup = function(
  ...
  var groups = this.hash.project.objects['PBXGroup'],
    ...
    referenceProxySection = this.pbxReferenceProxySection(),
    filePathToReference = {},
    buildFilePathToReference = {},
    referenceProxyPathToReference = {}
  ...
  for (var key in buildFileSection) {
    ...
    var fileReferenceKey = key.split(COMMENT_KEY)[0],
      buildFile = buildFileSection[fileReferenceKey]

    buildFilePathToReference[buildFile.fileRef_comment] = {
      fileRef: buildFile.fileRef,
      basename: buildFile.fileRef_comment,
    }
  }

  for (var key in referenceProxySection) {
    // only look for comments
    if (!COMMENT_KEY.test(key)) continue

    var referenceKey = key.split(COMMENT_KEY)[0],
      referencyProxy = referenceProxySection[referenceKey]

    referenceProxyPathToReference[referencyProxy.path] = {
      fileRef: referenceKey,
      basename: referencyProxy.path,
    }
  }
  ...
  for (var index = 0; index < filePathsArray.length; index++) {
    var filePath = filePathsArray[index],
      ...
      filePathBaseName = path.basename(filePath)
    if (filePathToReference[filePath]) {
      ...
      continue
    } else if (buildFilePathToReference[filePath]) {
      pbxGroup.children.push(pbxGroupChild(buildFilePathToReference[filePath]))
      continue
    } else if (referenceProxyPathToReference[filePath]) {
      pbxGroup.children.push(
        pbxGroupChild(referenceProxyPathToReference[filePath])
      )
      continue
    }

    if (!noFileRefNorBuildFile) {
...
pbxProject.prototype.addTargetDependencies = function({
  /**
   * Absolute or relative path to source xcodeproj (not pbxproj)
   * Required
   **/
  sourceProjectPath,
  /**
   * Array of source native targets name to be added as target dependencies
   * Required
   * TODO : Make it optional and consider all source native targets if ommitted
   **/
  sourceNativeTargets,
  /**
   * Array of destination native targets name to add the target dependency to
   * Optional
   * If ommitted, will add to add native targets in destination
   **/
  destNativeTargets,
}) {
  if (!sourceProjectPath) {
    throw new Error(
      '[addTargetDependencyEx] Missing required sourceProjectPath argument'
    )
  }

  if (!sourceNativeTargets) {
    throw new Error(
      '[addTargetDependencyEx] Missing required sourceNativeTargets argument'
    )
  }

  let destNativeTargetsIds
  if (!destNativeTargets) {
    destNativeTargetsIds = Object.keys(this.pbxNativeTargetSection()).filter(
      k => !COMMENT_KEY.test(k)
    )
  } else {
    destNativeTargetsIds = destNativeTargets.map(d => this.findTargetKey(d))
  }

  const pathToSourceProject = path.isAbsolute(sourceProjectPath)
    ? path.join(sourceProjectPath, 'project.pbxproj')
    : path.resolve(
  ...
  const sourceProject = this.openProjectSync(pathToSourceProject)

  const sourceProjectXCodeProjFileName = path.basename(sourceProjectPath)
  const files = nonComments(this.pbxFileReferenceSection())
  let file
  let fileReferenceId
  for (fileReferenceId in files) {
    file = files[fileReferenceId]
    if (file.name === sourceProjectXCodeProjFileName) {
      break
    }
  }
  ...
  for (const sourceNativeTarget of sourceNativeTargets) {
    ...
    const itemProxy = {
      ...
      containerPortal: fileReferenceId,
      containerPortal_comment: path.basename(file.path).replace('"', ''),
      proxyType: 1,
      remoteGlobalIDString: sourceProject.findTargetKey(sourceNativeTarget),
    ...
    if (pbxContainerItemProxySection && pbxTargetDependencySection) {
      ...
      pbxContainerItemProxySection[itemProxyCommentKey] =
        'PBXContainerItemProxy'
      pbxTargetDependencySection[targetDependencyUuid] = targetDependency
      pbxTargetDependencySection[targetDependencyCommentKey] =
        'PBXTargetDependency'
      for (const destNativeTargetId of destNativeTargetsIds) {
        nativeTargets[destNativeTargetId].dependencies.push({
...
pbxProject.prototype.addTargetDependency = function(
  ...
  for (var index = 0; index < dependencyTargets.length; index++) {
    ...
    var dependencyTargetUuid = remoteGlobalIDString || this.generateUuid(),
...
pbxProject.prototype.addCopyfileFrameworkCustom = function(name) {
  ...
  var sources = this.buildPhaseObject(
    'PBXCopyFilesBuildPhase',
    'CopyFiles',
    this.findTargetKey('ElectrodeContainer')
  )
  sources.files.push(pbxBuildPhaseObj(file))
}
...
pbxProject.prototype.addProject = function(
  projectPath,
  group,
  target,
  options
) {
  // Add xcodeproj file to PBXFileReference and PBXGroup
  var projectFile = this.addFile(
    projectPath,
    group ? this.findPBXGroupKey({ name: group }) : this.getMainPbxGroupUuid(),
    undefined
  )

  if (!options) {
    const pathToProjectToAdd = path.isAbsolute(projectPath)
    ...
    const projectToAdd = this.openProjectSync(pathToProjectToAdd)
    const nativeTargets = Object.values(
      projectToAdd.pbxNativeTargetSection()
    ).filter(n => typeof n !== 'string')
    const referenceProxyUUids = []
    for (const nativeTarget of nativeTargets) {
      var pbxContainerItemProxy = this.addPbxContainerItemProxy(
        projectFile,
        2,
        nativeTarget.productReference,
        nativeTarget.name
      )
      const file = new pbxFile(nativeTarget.productReference_comment)
      file.uuid = this.generateUuid()
      file.fileRef = this.generateUuid()
      const { uuid } = this.addPbxReferenceProxy(
        file,
        pbxContainerItemProxy.uuid
      )
      referenceProxyUUids.push(uuid)
    }
    const pbxGroup = this.addPbxGroupWithReferenceProxyUuids({
      referenceProxyUuids: referenceProxyUUids,
      groupName: 'Products',
    })
    const productGroup = { uuid: pbxGroup.uuid, name: 'Products' }
    ...
    if (options.staticLibs && options.staticLibs.length > 0) {
      ...
      for (var staticLib of options.staticLibs) {
        ...
        if (!staticLib.productReference) {
          var proj = this.openProjectSync(options.projectAbsolutePath)
          var pbxTarget = proj.pbxTargetByName(staticLib.target)
          uuidInContainingProject = pbxTarget.productReference
        } else {
          uuidInContainingProject = staticLib.productReference
        }

        var pbxContainerItemProxy = this.addPbxContainerItemProxy(
          projectFile,
          2,
          uuidInContainingProject,
          staticLib.target
        )
        this.addPbxReferenceProxy(file, pbxContainerItemProxy.uuid)
      }

      // Create group
      var staticLibsNames = options.staticLibs.map(function(elt) {
        return elt.name
      })
      var pbxGroup = this.addPbxGroup(staticLibsNames, 'Products')
      var productGroup = { uuid: pbxGroup.uuid, name: 'Products' }
      this.addProjectReference(productGroup, projectFile)
    }

    if (options.frameworks && options.frameworks.length > 0) {
      // Add each framework
      for (var framework of options.frameworks) {
        var file = new pbxFile(framework.name)
        if (this.hasFile(file.path)) continue
        file.uuid = this.generateUuid()
        file.fileRef = this.generateUuid()
        ...
        if (!framework.productReference) {
          var proj = this.openProjectSync(options.projectAbsolutePath)
          var pbxTarget = proj.pbxTargetByName(framework.target)
          uuidInContainingProject = pbxTarget.productReference
        } else {
          uuidInContainingProject = framework.productReference
        }
      ...
      // Create group
      var frameworksNames = options.frameworks.map(function(elt) {
        return elt.name
      })
      var pbxGroup = this.addPbxGroup(frameworksNames, 'Products')
      var productGroup = { uuid: pbxGroup.uuid, name: 'Products' }
      this.addProjectReference(productGroup, projectFile)
    }
  }
...
pbxProject.prototype.openProjectSync = function(projectPath) {
  const proj = new pbxProject(projectPath)
  return proj.parseSync()
}

pbxProject.prototype.addPbxContainerItemProxy = function(
  file,
  proxyType,
  remoteGlobalIDString,
  remoteInfo
) {
  var pbxContainerItemProxySection = this.hash.project.objects[
    'PBXContainerItemProxy'
  ]
  var itemProxyUuid = this.generateUuid()
  var itemProxyCommentKey = f('%s_comment', itemProxyUuid)

  var itemProxy = {
    isa: 'PBXContainerItemProxy',
    containerPortal: file.fileRef,
    containerPortal_comment: file.basename,
    proxyType: 2,
  ...
  return { uuid: itemProxyUuid, itemProxy: itemProxy }
}

pbxProject.prototype.addPbxReferenceProxy = function(file, remoteRef) {
  var pbxReferenceProxySection = this.hash.project.objects['PBXReferenceProxy']
  var referenceProxyCommentKey = f('%s_comment', file.fileRef)

  var referenceProxy = {
    isa: 'PBXReferenceProxy',
    fileType: file.lastKnownFileType,
    path: file.basename,
    remoteRef: remoteRef,
  ...
  pbxReferenceProxySection[file.fileRef] = referenceProxy
  pbxReferenceProxySection[referenceProxyCommentKey] = file.basename

  return { uuid: file.fileRef, referenceProxy: referenceProxy }
}
...
pbxProject.prototype.hasFile = function(filePath) {
  ...
  for (id in files) {
    ...
    if (file.path == filePath || file.path == '"' + filePath + '"') {
      file.fileRef = id
      const commentKey = f('%s_comment', id)
      file.basename = this.pbxFileReferenceSection()[commentKey]
      console.log(
        `hasFile ${filePath}. file is ${JSON.stringify(file, null, 2)}`
      )
      return file
    }
  }
  ...
  var buildConfigurationsList = [
    {
      ...
      buildSettings: {
        ...
        INFOPLIST_FILE:
          '"' +
          path.join(targetSubfolder, targetSubfolder + '-Info.plist' + '"'),
...
/**
 * Get the UUID of the main PBXGroup, from the first project found
 */
pbxProject.prototype.getMainPbxGroupUuid = function() {
  return this.getFirstProject().firstProject.mainGroup
}

/**
 * Get the main PBXGroup of the first project found
 */
pbxProject.prototype.getMainPbxGroup = function() {
  const mainPbxGroupUuid = this.getMainPbxGroupUuid()
  return this.getPBXGroupByKey(mainPbxGroupUuid)
}
...
pbxProject.prototype.addProjectReference = function(productGroup, projectRef) {
  if (
    !this.pbxProjectSection()[this.getFirstProject()['uuid']][
      'projectReferences'
    ]
  ...
  var projectReference = {
    ProjectRef: projectRef.fileRef,
    ProjectRef_comment: projectRef.basename,
    ProductGroup: productGroup.uuid,
    ProductGroup_comment: productGroup.name,
  }

  this.pbxProjectSection()[this.getFirstProject()['uuid']][
    'projectReferences'
  ].push(projectReference)
  //}
}
...
pbxProject.prototype.addFile = function(path, group, opt) {
  ...
  // return file if it already exist
