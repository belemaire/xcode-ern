...
pbxProject.prototype.addProject = function(projectAbsolutePath, projectPath, group, target, staticLibs) {
    // Add project
    ...
    if (staticLibs && staticLibs.length > 0) {
        // Add each static lib
        for (var staticLib of staticLibs) {
            ...
            this.addToPbxFileReferenceSection(file);    // PBXFileReference
            this.addToPbxFrameworksBuildPhase(file);    // PBXFrameworksBuildPhase
            var proj = this.openProjectSync(projectAbsolutePath)
            var pbxTarget = proj.pbxTargetByName(staticLib.target)
            var uuidInContainingProject = pbxTarget.productReference
        ...
        // Create group
        var staticLibsNames = staticLibs.map(function(elt) { return elt.name })
...
pbxProject.prototype.addFile = function (path, group, opt) {
    ...
    // null is better for early errors
    if (this.hasFile(file.path)) {
      console.log(`already there ${path}`)
      return null;
    }
