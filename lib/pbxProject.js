...
pbxProject.prototype.addTargetDependencies = function({
  ...
  if (!sourceProjectPath) {
    throw new Error('[addTargetDependencyEx] Missing required sourceProjectPath argument')
  }

  if (!sourceNativeTargets) {
    throw new Error('[addTargetDependencyEx] Missing required sourceNativeTargets argument')
  }

  let destNativeTargetsIds
  if (!destNativeTargets) {
    destNativeTargetsIds = Object.keys(this.pbxNativeTargetSection())
  ...
  const pathToSourceProject = path.isAbsolute(sourceProjectPath)
      ...
      : path.resolve(path.dirname(this.filepath), '..', sourceProjectPath, 'project.pbxproj')
...
pbxProject.prototype.addProject = function(
  ...
  if (!options) {
    const pathToProjectToAdd = path.isAbsolute(projectPath)
      ...
      : path.resolve(path.dirname(this.filepath), '..', projectPath, 'project.pbxproj')
    const projectToAdd = this.openProjectSync(pathToProjectToAdd)
    const nativeTargets = Object.values(projectToAdd.pbxNativeTargetSection())
    ...
    // Create group
    var elementNames = nativeTargets.map(function(elt) {
      return elt.productReference_comment
    })
    var pbxGroup = this.addPbxGroup(elementNames, 'Products', undefined, undefined, true)
