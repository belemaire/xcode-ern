...
pbxProject.prototype.addPbxGroup = function(filePathsArray, name, p, sourceTree) {
  ...
  for (var key in buildFileSection) {
    ...
    buildFilePathToReference[buildFile.fileRef_comment] = { fileRef: buildFile.fileRef, basename: buildFile.fileRef_comment };
  }
...
pbxProject.prototype.addProject = function(projectPath, group, target, options) {
var projectFile = this.addFile(projectPath, this.findPBXGroupKey({name: group}), undefined)
...
if (options.staticLibs && options.staticLibs.length > 0) {
  ...
  for (var staticLib of options.staticLibs) {
    ...
    var pbxContainerItemProxy = this.addPbxContainerItemProxy(projectFile, 2, uuidInContainingProject, staticLib.target)
    this.addPbxReferenceProxy(file, pbxContainerItemProxy.uuid)
  }
  
  // Create group
  var staticLibsNames = options.staticLibs.map(function(elt) { return elt.name })
...
if (options.frameworks && options.frameworks.length > 0) {
  ...
  for (var framework of options.frameworks) {
    ...
    var pbxContainerItemProxy = this.addPbxContainerItemProxy(projectFile, 2, uuidInContainingProject, framework.target)
    this.addPbxReferenceProxy(file, pbxContainerItemProxy.uuid)
  }
  
  // Create group
  var frameworksNames = options.frameworks.map(function(elt) { return elt.name })
...
pbxProject.prototype.addPbxContainerItemProxy = function(file, proxyType, remoteGlobalIDString, remoteInfo) {
...
pbxProject.prototype.addProjectReference = function(productGroup, projectRef) {
  if (!this.pbxProjectSection()[this.getFirstProject()['uuid']]['projectReferences']) {
    this.pbxProjectSection()[this.getFirstProject()['uuid']]['projectReferences'] = [];
  }
  //if (!this.hasKnownRegion(name)) {
  ...
  this.pbxProjectSection()[this.getFirstProject()['uuid']]['projectReferences'].push(projectReference);
  //}
}
